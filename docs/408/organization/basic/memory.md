...
CPU 速度远大于内存速度，导致内存成为性能瓶颈，双端口和多模块并不能很大解决这一问题
...
直接映射：主存标记 + cache 块号 + cache 块内地址

- cache 块内地址位数 = log2(cache块大小)
- cache 块号位数 = log2(cache块数)
- 主存总位数 = log2(主存大小)
- 主存标记位数 = 主存总位数 - cache块号位数 - cache 块内地址位数

全相联映射：主存标记 + cache 块内地址

- 主存标记位数 = 主存总位数 - cache 块内地址位数

组相联映射：主存标记 + cache 组号 + cache 块内地址

- cache 块内地址位数 = log2(cache块大小)
- cache 组号位数 = log2(cache组数)
- cache 组数 = cache 块数 / n路组相联
- 主存总位数 = log2(主存大小)
- 主存标记位数 = 主存总位数 - cache组号位数 - cache 块内地址位数
