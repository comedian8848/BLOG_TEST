...
- 首先通过主存块数 j 和 cache 组数 n 进行映射，将主存先映射到组 j % n，这一步和直接映射一模吊样，进行哈希散列
- 教材中是模的 cache 块数，不是组数，但考试中大多以第一种考
- 再在 cache 组内进行自由映射
...
主存-cache系统解决内存的速度问题，虚拟存储器系统解决内存大小问题

虚拟存储器由主存储器和辅助存储器联机工作共同组成，在逻辑是形成一个统一的存储器
...
将主存和辅存地址空间统一编址，形成一个统一的存储空间，用户在这个逻辑地址中自由编程
...
将进程的逻辑空间分为**若干大小相等**的片，称为页，以 0 开始编号，在内存空间中，同样以这种**形式及大小**进行分片并编号，注意在内存中每片均是单例，为进程所共用，进程之间相互独立，也就是说有多个进程第 0 片，但只有一个内存第 0 片
...
其中物理地址和缓存地址的长度一样，逻辑地址和物理地址长度不一，其映射关系储存在页表中（因为页内地址一样，映射的实际上只有前面的页框号，页内地址直接拼接上即可）
...
- cache 解决主存和 CPU 速度差异；虚拟存储器弥补主存和辅存的容量差距

- cache 传输数据的基本单位为块，较小；虚拟存储器以页为单位，长度为几百~几百K字节

- CPU 和 cache 以及主存都有直接访问的通道；而 CPU 和辅存之间没有，必须通过主存调块

- cache 存取信息的过程均由硬件实现，对程序员是透明的；而虚拟存储器依赖于操作系统的存储管理软件，对系统程序员并非透明
